        ...
        body: Column(
          ...
          children: <Widget> [
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                style: ButtonStyle(
                    backgroundColor:MaterialStateProperty.all(GeneralUiConfig.btnColor),
                ...
                onPressed: () async {
                  ...
                  // TODO: 이미 해당날짜에 달똥약속이 있거나 블락한 유저가 있는지 체크.
                  context.read<DalddongProvider>().newDdFriends.forEach((eachUser) {
                    lunchOrDinner = context.read<DalddongProvider>().DalddongLunch ? 0 : 1;
                    FirebaseFirestore.instance
                        .collection('user')
                        .doc(eachUser.get('userEmail'))
                        .collection('BlockDatesList')
                        .snapshots().forEach((blockSnapshot) {
                      blockSnapshot.docs.forEach((blockDate) {
                        if(DateTime.parse(blockDate.id) == context.read<DalddongProvider>().DalddongDate &&
                           blockDate.get('LunchOrDinner') == lunchOrDinner){
                          showAlertDialog(context, "${context.read<DalddongProvider>().DalddongDate} 에 이미 달똥약속이 존재하는 인원이 있습니다. 날짜를 바꿔보세요!");
                          isError = true;
                          return;
                        }
                      });
